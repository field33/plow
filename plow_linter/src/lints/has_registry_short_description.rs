...
// Identical to the length of a twitter tweet
...
impl Lint for HasRegistryShortDescription {
    ...
    /// Lints for the existence of `registry:shortDescription` and its correct format
            ...
            if let Some(root_prefix) = get_root_prefix(document) {
                ...
                let result = annotation.object().as_literal().map_or_else(
                    ...
                    |literal| {
                        ...
                        if license_raw.chars().count() > SHORT_DESCRIPTION_MAX_ALLOWED_CHAR_COUNT {
